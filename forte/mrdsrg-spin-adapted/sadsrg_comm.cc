...
std::vector<double> SADSRG::H2_T2_C0_T2small_sym(BlockedTensor& H2_sym,  BlockedTensor& T2,
    ...
    // HH
    // temp["uvxy"] += 0.5 * H2["uvmn"] * T2["mnxy"];
    temp["a7,a6,x,y,"] += 0.5 * H2_sym["a7,a6,c9,c8,"] * T2["c9,c8,x,y,"];//aacc
    // temp["uvxy"] += 0.5 * H2["uvmw"] * T2["mzxy"] * L1_["wz"];
    temp["a7,a6,x,y,"] += 0.5 * H2_sym["a7,a6,c9,a8,"] * T2["c9,z,x,y,"] * L1_["a8,z,"];//aaca

    // PP
    // temp["uvxy"] += 0.5 * H2["efxy"] * T2["uvef"];
    temp["u,v,a6,a7,"] += 0.5 * H2_sym["v8,v9,a6,a7,"] * T2["u,v,v8,v9,"];//vvaa
    // temp["uvxy"] += 0.5 * H2["ezxy"] * T2["uvew"] * Eta1_["wz"];
    ...
    if (do_cu3_) {
        if (store_cu3_) {
            ...
            // - need to transform the integrals to the same orbital basis as active space solver
            // - TODO: maybe we (York) should make the CI vectors consistent at the first place
            std::cout<<"Direct algorithm for 3rdm";
            // ambit::Tensor Tbra, Tket;
            // ambit::Tensor Ua = Uactv_.block("aa");
...
void SADSRG::H2_T1_C1_sym(BlockedTensor& H2_sym,BlockedTensor& T1, const double& alpha,
                      BlockedTensor& C1) {
    local_timer timer;
    // C1["qp"] += 2.0 * alpha * T1["ma"] * H2["qapm"];
...
C1["v2,a,"] += 0.5 * alpha * Eta1_["a9,v,"] * S2["c1,v,a,v7,"] * H2_sym["v2,v7,c1,a9,"];//vvca
    ...
    // C1["pa"] += 0.25 * alpha * S2["vjax"] * Eta1_["uv"] * Eta1_["xy"] * H2["pyuj"];
C1["a2,a,"] += 0.25 * alpha * S2["v,a0,a,x,"] * Eta1_["a9,v,"] * Eta1_["x,a8,"] * H2_sym["a2,a8,a9,a0,"];//aaaa
C1["a2,a,"] += 0.25 * alpha * S2["v,c0,a,x,"] * Eta1_["a9,v,"] * Eta1_["x,a8,"] * H2_sym["a8,a2,c0,a9,"];//aaca****
C1["v2,a,"] += 0.25 * alpha * S2["v,a0,a,x,"] * Eta1_["a9,v,"] * Eta1_["x,a8,"] * H2_sym["a8,v2,a0,a9,"];//avaa****
C1["v2,a,"] += 0.25 * alpha * S2["v,c0,a,x,"] * Eta1_["a9,v,"] * Eta1_["x,a8,"] * H2_sym["a8,v2,c0,a9,"];//avca****
C1["c2,a,"] += 0.25 * alpha * S2["v,a0,a,x,"] * Eta1_["a9,v,"] * Eta1_["x,a8,"] * H2_sym["c2,a8,a9,a0,"];//caaa
C1["c2,a,"] += 0.25 * alpha * S2["v,c0,a,x,"] * Eta1_["a9,v,"] * Eta1_["x,a8,"] * H2_sym["c2,a8,a9,c0,"];//caac
    // C1["pa"] += 0.25 * alpha * S2["jvax"] * Eta1_["xy"] * Eta1_["uv"] * H2["pyju"];
